...
class CommunicationHub(Document):
	...
	def validate(self):
		...
		if self.channel == "Voice" and not self.vapi_call_id:
			frappe.throw(_("VAPI Call ID is required for Voice channel"))

		if self.channel == "Email" and not self.email_message_id:
			frappe.throw(_("Email Message-ID is required for Email channel"))

		if self.channel in ["Facebook", "Instagram"] and not self.social_sender_id:
			frappe.throw(_("Sender Platform ID is required for {0}").format(self.channel))

		if self.channel == "Chat" and not self.chatwoot_conversation_id:
			frappe.throw(_("Chatwoot Conversation ID is required for Chat channel"))
...
def create_from_webhook(data):
	...
	hub = frappe.get_doc({
		...
		"vapi_phone_number": data.get("phone_number"),
