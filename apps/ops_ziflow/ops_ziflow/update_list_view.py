...
script = '''frappe.listview_settings["OPS ZiFlow Proof"] = {
...
def run():
    cs = frappe.get_doc("Client Script", "OPS ZiFlow Proof List View")
    cs.script = script
    cs.save()
    frappe.db.commit()
    print("Updated: Thumbnail in ZiFlow Proof ID column, ID column hidden!")
