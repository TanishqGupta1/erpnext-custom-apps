"""
OPS Product Margin Visualizer - Fixed Version
Now properly uses product_pages_data field instead of custom_size_info
"""
import frappe

def create_custom_field():
    """Create custom field for product pages data if it doesn't exist"""

    # Check if custom field already exists
    existing = frappe.db.exists("Custom Field", {
        "dt": "OPS Product",
        "fieldname": "product_pages_data"
    })

    if existing:
        print(f"Custom field 'product_pages_data' already exists")
        return existing
    ...
    doc = frappe.get_doc({
        ...
        "dt": "OPS Product",
        "fieldname": "product_pages_data",
        "label": "Product Pages Data",
        "fieldtype": "Small Text",
        "insert_after": "productpages",
        "hidden": 1,
        "description": "JSON data for product pages (name, sort, active)"
    })
    doc.insert()
    frappe.db.commit()
...
def create_client_script():
    ...
    script_content = '''/**
 * OPS Product Margin Visualizer - Fixed Version
 * Uses product_pages_data for pages instead of custom_size_info
 */
...
<div class="ops-pages-box">
    ...
    render: function() {
        ...
        // Hide the new custom field too if it exists
        if (frm.fields_dict.product_pages_data) {
            $(frm.fields_dict.product_pages_data.wrapper).hide();
        }
    ...
    // FIXED: Now uses product_pages_data instead of custom_size_info
    getPages: function() {
        // Use product_pages_data field for pages (NOT custom_size_info)
        const json = this.frm.doc.product_pages_data;
    ...
    // FIXED: Now saves to product_pages_data instead of custom_size_info
    savePages: function() {
        const pages = [];
        ...
        // Save to product_pages_data (NOT custom_size_info)
        this.frm.set_value('product_pages_data', JSON.stringify(pages));
...
// Sort child table by sort_order field
function sort_child_table_by_sort_order(frm, table_fieldname) {
    if (!frm.doc[table_fieldname] || frm.doc[table_fieldname].length === 0) return;

    // Sort the array by sort_order
    frm.doc[table_fieldname].sort((a, b) => {
        const sortA = parseInt(a.sort_order) || 0;
        const sortB = parseInt(b.sort_order) || 0;
        return sortA - sortB;
    });

    // Update idx values to match new order
    frm.doc[table_fieldname].forEach((row, index) => {
        row.idx = index + 1;
    });

    // Refresh the grid to show sorted order
    if (frm.fields_dict[table_fieldname] && frm.fields_dict[table_fieldname].grid) {
        frm.fields_dict[table_fieldname].grid.refresh();
    }
}
...
frappe.ui.form.on('OPS Product', {
    refresh: function(frm) {
        if (frm.doc.name) {
            ...
            // Sort product_options by sort_order
            sort_child_table_by_sort_order(frm, 'product_options');
        }
    },
    onload: function(frm) {
        if (frm.doc.name) {
...
'''
    ...
    # Update or create Client Script
    ...
    frappe.db.commit()
    return "Fixed version deployed - now uses product_pages_data field!"
