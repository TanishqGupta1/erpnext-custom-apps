...
class OPSZiFlowProof(Document):
    ...
    def apply_ziflow_payload(self, payload: dict[str, Any] | None) -> None:
        ...
        self.ziflow_folder_id = payload.get("folder_id") or folder.get("id") or self.ziflow_folder_id
        self.folder_name = folder.get("name") or self.folder_name
        template = payload.get("template") or {}
        self.template_id = payload.get("template_id") or template.get("id") or self.template_id
        self.template_name = template.get("name") or self.template_name
        self.versions_json = (
